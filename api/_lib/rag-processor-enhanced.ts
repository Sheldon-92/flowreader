/**
 * Enhanced RAG Processing Implementation
 *
 * Updated version of the RAG processor that integrates the new multi-layer
 * caching system for improved performance while maintaining full compatibility.
 *
 * This can be used as a drop-in replacement for the existing rag-processor.ts
 * or deployed gradually using feature flags.
 */
...
import { getEnhancedCacheManager, type EnhancedCacheInterface } from './enhanced-cache-integration.js';
...
export class EnhancedRAGProcessor {
  ...
  private enhancedCache: EnhancedCacheInterface = getEnhancedCacheManager();
  ...
  async processBookForRAG(bookId: string): Promise<{
    ...
    try {
      ...
      // Generate embeddings with enhanced caching
      const embeddings = await this.generateEmbeddingsWithCache(allTexts, bookId);
      ...
      // Pre-warm cache with popular queries for this book
      await this.preWarmBookCache(bookId);
  ...
  /**
   * Generate streaming AI response with enhanced caching
   */
  async *streamChatResponse(request: ChatStreamRequest): Promise<AsyncGenerator<{
    ...
    try {
      console.log(`🤖 Starting enhanced chat stream for book ${request.book_id}`);

      // Check enhanced cache first
      const cachedResponse = await this.enhancedCache.getCachedResponse({
      ...
      if (cachedResponse) {
        console.log(`✨ Enhanced cache hit: ${cachedResponse.cacheLayer}, fresh: ${cachedResponse.fresh}`);
        ...
        for (let i = 0; i < words.length; i += 3) {
          ...
          await new Promise(resolve => setTimeout(resolve, cachedResponse.fresh ? 5 : 10)); // Faster for fresh cache
        }

        // Emit enhanced usage information
        yield {
          type: 'usage',
          data: {
            ...
            cached: true,
            cache_layer: cachedResponse.cacheLayer,
            fresh: cachedResponse.fresh,
            stale: cachedResponse.stale
          }
        };

        yield {
          type: 'done',
          data: { completed_at: Date.now(), cached: true, enhanced: true }
        };

        return;
      }

      // Check for cached embedding with enhanced system
      let queryEmbedding = await this.enhancedCache.getCachedEmbedding(request.message);
      ...
      if (!queryEmbedding) {
        ...
        // Cache the embedding in enhanced system
        await this.enhancedCache.cacheEmbedding(request.message, queryEmbedding);
      }
      ...
      // Cache the response in enhanced system
      await this.enhancedCache.setCachedResponse({
        bookId: request.book_id,
        message: request.message,
        selection: request.selection,
        chapterIdx: request.chapter_idx,
        ...
        metadata: {
          model,
          budgetStrategy: budgetOptimization.budget.strategy,
          optimizationApplied: optimizationStrategies.length > 0
        }
      });
      ...
      yield {
        type: 'done',
        data: { completed_at: Date.now(), enhanced: true }
      };

      console.log(`✅ Enhanced chat stream completed with ${totalTokens} tokens, quality score: ${qualityMetrics.overallQuality.toFixed(2)}`);

    } catch (error) {
      console.error('Enhanced chat stream failed:', error);
      yield {
        type: 'error',
        data: {
          code: 'ENHANCED_CHAT_STREAM_ERROR',
  ...
  // Enhanced cache-specific methods

  /**
   * Generate embeddings with intelligent caching
   */
  private async generateEmbeddingsWithCache(texts: string[], bookId: string): Promise<number[][]> {
    ...
    for (let i = 0; i < texts.length; i++) {
      const cached = await this.enhancedCache.getCachedEmbedding(texts[i]);
      if (cached) {
        embeddings[i] = cached;
      } else {
    ...
    console.log(`📊 Embedding cache: ${texts.length - uncachedTexts.length}/${texts.length} cached`);

    // Generate missing embeddings
    if (uncachedTexts.length > 0) {
      const newEmbeddings = await this.embeddingGenerator.generateEmbeddings(uncachedTexts);
      ...
      for (let i = 0; i < uncachedTexts.length; i++) {
        ...
        await this.enhancedCache.cacheEmbedding(uncachedTexts[i], embedding);
      }
    }
  ...
  /**
   * Pre-warm cache with popular book queries
   */
  private async preWarmBookCache(bookId: string): Promise<void> {
    ...
    try {
      await this.enhancedCache.preWarmCache(bookId, popularQueries);
      console.log(`🔥 Pre-warmed cache for book ${bookId} with ${popularQueries.length} popular queries`);
    } catch (error) {
      console.warn('Cache pre-warming failed:', error);
    }
  }

  // All the existing private methods remain the same...
  // (Copying the private methods from the original implementation)
...
Remember: You're having a conversation with a reader about their book, not writing an essay.`;
  ...
  private async saveConversation(
    ...
    try {
      ...
      console.log(`💾 Saving enhanced conversation for book ${request.book_id}`);
  ...
  // Include all other private methods from the original implementation...
  ...
  private optimizeContextWindow(
    ...
    console.log(`📊 Enhanced context optimization: ${chunks.length} chunks → ${optimized.length} chunks (${currentTokens} tokens)`);
